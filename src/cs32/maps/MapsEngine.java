...
public class MapsEngine {
	...
	protected String getOutputFromLatLongs(List<Double> latlons) throws IOException {
		// find nearest start node (kdtree)
		String nearestStartNode = k.searchNumber(1, new Coordinates(latlons.get(0), latlons.get(1))).get(0);
		// find nearest end node (kdtree)
		String nearestEndNode = k.searchNumber(1, new Coordinates(latlons.get(2), latlons.get(3))).get(0);
		// get shortest path between them (PathFinder)
		return null;
	}
